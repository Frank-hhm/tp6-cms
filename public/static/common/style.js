/*
* @Author: 小明
* @Date:   2019-12-30 01:22:00
* @Last Modified by:   根子科技
* @Last Modified time: 2020-02-22 20:50:22
*/
...
require.config({
    ...
    paths: {
        ...
        'dragsort': ['../plugs/dragsort/dragsort'],
        'message': ['../plugs/message/my-message'],
        'formSelects': ['../plugs/formSelects/formSelects-v4.min']
    },
    shim: {
        'websocket': {deps: [ '../plugs/socket/swfobject.min.js']},
        'jquery.ztree': {deps: ['jquery', 'css!' +  '../plugs/ztree/zTreeStyle/zTreeStyle.css']},
        'jquery.autocompleter': {deps: ['jquery', 'css!' +  '../plugs/jquery/autocompleter.css']},
    }
});
...
$(function(){
    ...
    $.msg = new function (that) {
        ...
        this.confirm = function (msg, ok, no) {
            var index = layer.confirm(msg, {title:false, btn: ['确认', '取消']}, function () {
        ...
        this.success = function (msg, time, callback) {
            require(['message'], function (_$) {
                _$.add(msg, 'success',(time || 1) * 1000,callback);
            });
        };
        // 显示失败类型的消息
        this.error = function (msg, time, callback) {
            require(['message'], function (_$) {
                _$.add(msg,'error', (time || 1) * 1000,callback);
            });
        };
        // 状态消息提示
        ...
        this.auto = function (ret, time,callback=null) {
            ...
            $('[data-btn-loaded] i[loaded]').remove();
    ...
    $.menu = new function (that) {
        ...
        this.listen = function () {
            // 菜单模式切换
            (function ($menu, miniClass,_i) {
                // Mini 菜单模式切换及显示
                if (layui.data('admin-menu-type')['type-min']){
                     $menu.addClass(miniClass)
                     $('[data-target-menu-type]').find('i').attr('class',_i)
                };
                $body.on('click', '[data-target-menu-type]', function () {
                    ...
                    layui.data('admin-menu-type')['type-min']? _i ='layui-icon layui-icon-spread-left' : _i ='layui-icon layui-icon-shrink-right';
                    $('[data-target-menu-type]').find('i').attr('class',_i)
                }).on('resize', function () {
                    if ($body.width() > 1000) {
                        layui.data('admin-menu-type')['type-min'] ? _i ='layui-icon layui-icon-spread-left' : _i ='layui-icon layui-icon-shrink-right';
                        layui.data('admin-menu-type')['type-min'] ? $menu.addClass(miniClass) : $menu.removeClass(miniClass);
                         ...
                         _i ='layui-icon layui-icon-spread-left'
                        $menu.addClass(miniClass);
                    }
                    $('[data-target-menu-type]').find('i').attr('class',_i)
                }).trigger('resize');
            ...
            })($('.layui-layout-admin'), 'layui-layout-left-mini','layui-icon layui-icon-shrink-right');
            // 左则二级菜单展示
    ...
    $.vali = function (form, callback, options) {
        return (new function (that) {
            ...
            this.insertError = function (ele) {
                var $html = $('<span style="padding-right:12px;color:red;position:absolute;right:0;font-size:12px;z-index:2;display:block;width:34px;text-align:center;pointer-events:none"></span>');
            ...
            this.check = function (form, callback) {
                ...
                $(form).find('[data-btn-loaded]').map(function () {
                    $(this).html(this.getAttribute('data-btn-loaded') || this.innerHTML);
    ...
    $.form = new function (that) {
        ...
        this.reInit = function ($dom) {
            ...
            $dom.find('input[data-date-input]').map(function () {
                laydate.render({
                    ...
                    range:this.getAttribute('data-range')=='true'?true:false, 
                    elem: this,
                    theme: '#23262E', 
                    format:this.getAttribute('date-format')|| 'yyyy-MM-dd',
                    done: function (value) {
                        $(this.elem).val(value).trigger('change');
                    }
                });
            });
            $dom.find('[data-color]').map(function () {
                colorpicker.render({
                    elem:this
                    ,change: function(color){
                        $this = $(this.elem), field = $this.attr('data-field') || 'color';
            ...
            $dom.find('[data-upload-list]').map(function () {
                var attrs = $(this).attr("data-upload-list"),
                    inputName = $(this).attr("data-upload-name"),
                    fileValArr  = new Array(),
                    demoListView = $('['+attrs+']'),
                    uploadListIns = window.upload.render({
                    elem: '[data-upload-list]'
                    ,url:ROOT_URL + '/api.plugs/upload.html'
                    ,accept: 'file'
                    ,multiple: true
                    ,auto: false
                    ,bindAction: '[data-upload-list-action]'
                    ,choose: function(obj){   
                        var files = this.files = obj.pushFile();
                        if(files!=null &&files!={}){
                            $('.upload-list-box').show();
                        }else{
                            $('.upload-list-box').hide();
                        }
                        //读取本地文件
                        obj.preview(function(index, file, result){
                            var tr = $(['<tr id="upload-'+ index +'">'
                                  ...
                                  ,'<td class="text-blue">等待中...</td>'
                                  ,'<td>'
                                  ,'<button class="layui-btn layui-btn-xs bg-red upload-reload layui-hide">重传</button>'
                                  ,'<button class="layui-btn layui-btn-xs bg-gray upload-delete">删除</button>'
                                  ,'</td>'
                                  ,'</tr>'].join(''));
                            //单个重传
                            ...
                            tr.find('.upload-delete').on('click', function(){
                                delete files[index];
                                tr.remove();
                                if(files!=null && !isEmptyObject(files) ){
                                    $('.upload-list-box').show();
                                }else{
                                    if ( demoListView.html() == "") {$('.upload-list-box').hide();}
                                }
                    ...
                    ,done: function(res, index, upload){
                        if(res.uploaded){
                            ...
                            ,tds = tr.children();
                            tds.eq(2).html('<span class="text-green">上传成功</span>');
                            tds.eq(3).html("已上传"); 
                            fileValArr.push(res.filename);
                            $('[name="'+inputName+'"]').val(fileValArr);
        ...
        this.iframe = function (url, title, area) {
            return layer.open({title: title || '窗口', type: 2,skin: 'modal-class', area: area || ['800px', '580px'], fix: true, maxmin: false, content: url});
        };
        // 加载HTML到弹出层
        this.modal = function (url, data, title, callback, loading, tips,area) {
            this.load(url, data, 'GET', function (res, index) {
                ...
                index = layer.open({
                    type: 1, btn: false,anim:5,skin: 'modal-class',area: area||"800px", content: res, title: title || '', success: function (dom, index) {
    ...
    $.fn.formToJson = function () {
        var self = this, data = {}, pushCounters = {},
        	loaded = $(self).find('[data-btn-loaded]'),
        	loaded_html = $(self).find('[data-btn-loaded] i').attr('loaded'),
        	loadedIcon = '<i loaded class="layui-icon layui-icon-loading layui-icon layui-anim layui-anim-rotate layui-anim-loop"></i>';
        if(loaded_html!='') loaded.append(loadedIcon);
    ...
    $body.on('click', '[data-select]', function () {
        var field = $(this).attr('data-select') || $(this).attr('data-field') || 'id';
        var location = ROOT_URL + '/api.data/'+$(this).attr('data-select')+'.html?field=' + field;
        $.form.iframe(location, $(this).attr('data-title')||'选择数据',['800px','700px']);
    });
    ...
    $body.on('click', '[data-upload-img]', function () {
        var field = $(this).attr('data-upload-img') || $(this).attr('data-field') || 'image';
        var fileId = $(this).attr('data-fileId') || '';
        var location = ROOT_URL + '/api.plugs/files.html?field=' + field + (fileId!=''?"&fileId=" + fileId:'');
        $.form.iframe(location, '图片库',['900px','800px']);
    });
    /*! 注册 data-upload-mult-img 事件行为 */
    $body.on('click', '[data-upload-mult-img]', function () {
        var field = $(this).attr('data-upload-img') || $(this).attr('data-field') || 'image';
        var location = ROOT_URL + '/api.plugs/files.html?field=' + field + '&multiple=true';
        $.form.iframe(location, '图片库',['900px','800px']);
    });
    ...
    $body.on('click', '[data-modal]', function () {
        return $.form.modal($(this).attr('data-modal'), 'open_type=modal', $(this).attr('data-title') || $(this).text() || '编辑',null,null,null,$(this).attr('data-area'));
    });
    /*! 注册 data-open 事件行为 */
    ...
    /*! 注册 data-address 事件行为 */
    $body.on('click', '[data-address]', function () {
        var field = $(this).attr('data-address') || $(this).attr('data-field') || 'coordinate';
        var location = ROOT_URL + '/api.plugs/getpoint.html?field=' + field;
        $.form.iframe(location, '地址选择',['900px','800px']);
    });
    ...
    $body.on('click', '[data-action]', function () {
        ...
        if (rule.length < 1) return require(['message'], function (_$) {_$.add('请选择需要更改的数据！','error',100,null);});
    ...
    $body.on('blur', '[data-action-blur]', function () {
        ...
        that.callback = function (ret) {
            return (parseInt(ret.code) === 1) ?  ret.url ? (window.location.href = ret.url) : $.form.reload(): $this.css('border', (ret && ret.code) ? '1px solid #e6e6e6' : '1px solid red') ;
        };
        data['_csrf_'] = $this.attr('data-token') || $this.attr('data-csrf') || '--';
    ...
    $body.on('click', '[data-icon]', function (field, location) {
        ...
        location = ROOT_URL + '/api.plugs/icon.html?field=' + field;
    ...
    $body.on('click','[search-reload]',function(){
        var _this = $(this).siblings('[search-box]')
        _this.find('select').val('');
        _this.find('.layui-form-select input').val('');
        _this.find('.layui-form-select .layui-anim .layui-this').removeClass('layui-this');
        window.form.render();
    })
    ...
    $body.on('click', '[data-full-screen]', function () {
        ...
        if(!$('body').hasClass('full-screen')) {
            ...
            $('[data-full-screen]').addClass('active');
            // 判断浏览器设备类型
            ...
            $('[data-full-screen]').removeClass('active');
            //  退出全屏
    ...
    $.copyToClipboard = function (content, input) {
        ...
        document.body.appendChild(input), input.select(), setTimeout(function () {
            document.execCommand('Copy') ? $.msg.success('复制成功', 1000,null): $.msg.error('复制失败，请使用鼠标操作复制！', 1000,null) ;
    ...
    $body.find('[data-login-form]').map(function () {
        require(["md5"], function (md5) {
            $("form").vali(function (data) {
                ...
                $.form.load(location.href, data, "post", null, null, null, '2');
            });
        });
    });
    ...
    document.addEventListener('error', function (e, elem) {
        ...
        if (elem.tagName.toLowerCase() === 'img') {
            elem.src = baseRoot + 'img/404_icon.png';
        }
    }, true);
